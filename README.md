...
| `M_MED8_v1` | Parkinson's disease evidence chain: ETD vs institutional inertia competition rule test. dopamine_depletion_hypothesis (one of neurology's best-confirmed mechanisms) vs UPDRS endpoint (institutionally entrenched like PANSS in schizophrenia). Pre-registered competition rule: r>=0.40 ETD wins; r<0.20 institutional wins; 0.20-0.40 balanced. | **3/4 CONFIRMED, 1/4 denied** (see M_MED8 section). h1 CONFIRMED r(j,se)=+0.276P/+0.076S (balanced competition rule -- ETD contributes magnitudes, institutional inversion destroys rank structure); h2 CONFIRMED dopamine_depletion rank #1 justifies; h3 CONFIRMED dopamine_depletion rank #1 cites_as_support; h4 CONFIRMED dat_spect rank #2 selects_endpoints. Key findings: Pearson>>Spearman (MPI -- Magnitude-Preserving Inversion); r(se,cites)=-0.302 (measurement layer partially decoupled from citation loop); dopamine_depletion is structural keystone (potential disconnecting hub). 2026-06-04, hash `see M_MED8 file`. |
| `M_MED9_v1` | Focused Ultrasound (FUS) + microbubble BBB opening evidence chain: Mechanistically Grounded Surrogate Capture test. Strong physical mechanism (acoustic cavitation -> tight junction disruption) co-exists with gadolinium MRI surrogate endpoint that IS mechanism-derived. Competition between ETD signal and regulatory institutional substitution. | **3/4 CONFIRMED, 1/4 DENIED**. h1 DENIED r(j,se)=-0.067P/-0.081S p=0.840 (NOVEL finding: strong mechanism does NOT produce positive r(j,se) when regulatory framework controls endpoint selection orthogonal to theoretical chain -- Mechanistically Grounded Surrogate Capture); h2 CONFIRMED mri_gadolinium_endpoint rank #1 selects_endpoints (degree=4, dominant surrogate); h3 CONFIRMED fus_bbb_hypothesis rank #1 cites_as_support (HIGH EM: degree=4); h4 CONFIRMED acoustic_cavitation_mechanism rank #2 justifies. KEY STRUCTURAL DISCOVERIES: (1) regulatory_framework_fus: rank #12 justifies (zero theoretical standing) / rank #2 selects_endpoints (high institutional authority) / rank #1 BETWEENNESS (0.442, highest in network) -- Regulatory Betweenness Inversion: institutional actor is the structural keystone without theoretical content; (2) tight_junction_disruption: rank #3 justifies / rank #12 selects_endpoints / rank #12 cites -- compound citation+endpoint shadow (highest mechanistic importance, zero clinical visibility); (3) r(j,cites)=+0.335 (moderate -- mechanism tracks citations) but r(j,se)=-0.067 (near-zero -- mechanism does NOT track endpoint selection) -- structural decoupling between theoretical and institutional layers despite mechanistic validity. Three-axis model: EM=HIGH, ETD=NEAR-ZERO (institutional substitution), MSC=LOW. EM*(1-ETD) citation anomaly risk is HIGH -- preclinical data citations may be concentrated without adequate independent clinical validation. 2026-06-05, hash `22838135…`, commit [`4a8928c`](https://github.com/vladi160/preregistrations/commit/4a8928c) |
...
| `M_MED8_v1` | Parkinson's disease evidence chain: competition rule test -- ETD (endpoint theory-derivation) vs institutional endpoint inertia (UPDRS analogous to PANSS). Tests which structural axis wins when both mechanism quality and institutional history are high. | **3/4 CONFIRMED, 1/4 PARTIAL**. h1 PARTIAL r=+0.276P/+0.076S p=0.475 (competition rule = BALANCED: ETD wins at magnitude level, institutional inertia wins at rank level); h2 CONFIRMED dopamine_depletion rank#1 justifies (topological keystone -- removal may disconnect graph); h3 CONFIRMED dopamine_depletion rank#1 cites (EM=0.92); h4 CONFIRMED dat_spect_biomarker rank#3 selects (ETD present alongside institutional UPDRS). NEW: Pearson>Spearman inversion (institutional rank inversion, opposite of Alzheimer's amplification). NEW: r(se,cites)=-0.302 (endpoint and citation layers anti-correlated -- circularity resistance). Integrated pluralism: alpha-synuclein justifies dopamine (like Alzheimer's, not schizophrenia). Two citation shadows (nigrostriatal gap=8, dat_spect gap=5). Neuroprotection failure as structural moderator (absent from all circular chains). 2026-06-04, hash b24767c8, commit 6316e23. |
